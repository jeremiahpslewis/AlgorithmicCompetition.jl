...
for signals in signal_scenarios
    df_temp = @chain df_raw begin
        @transform(:signals = signals, :demand_signals = get_demand_signals(:is_high_demand, [true, false], signals[1], signals[2]))
    end
    append!(df, df_temp)
end
...
df = @chain df begin
    @transform(:identical = sum(:demand_signals) ∈ [0, 2], :identical_and_correct = all(:is_high_demand == :demand_signals))
    @transform(:minority_demand_state = (:is_high_demand & (:frequency_high_demand > 0.5)) | (!:is_high_demand & (:frequency_high_demand <= 0.5)))
...
df = @chain df_raw begin
    ...
    @transform(:post_prob_high_demand__cond_low_signal = :signal_strength * (1 - :frequency_high_demand) / :prob_low_signal)
...
plt_a_1 = @chain df begin
    data(_)
    mapping(:frequency_high_demand, :probability_combined, color=:signal_strength => nonnumeric, row=:signal) *
    visual(Lines)
end
draw(plt_a_1)
