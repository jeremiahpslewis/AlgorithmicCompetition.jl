"""
    construct_ACAI_state_space_lookup(action_space, n_prices)

Construct a lookup table from action space to the state space.
"""
function construct_ACAI_state_space_lookup(action_space, n_prices)
    @assert length(action_space) == n_prices^2 * 2 * 2
    state_space_lookup = reshape(Int64.(1:length(action_space)), n_prices, n_prices, 2)
    return state_space_lookup
end


"""
    construct_ACAI_profit_array(price_options, params, n_players)
