...
struct TDLearner{A,F<:AbstractFloat,I<:Integer} <: AbstractLearner
    approximator::A
    γ::F = 1.0
    method::Symbol
    n::I = 0
...
RLCore.estimate_reward(L::TDLearner{A,F,I}, s::I1) where {A,F,I1<:Integer} = RLCore.estimate_reward(L.approximator, s)
RLCore.estimate_reward(L::TDLearner{A,F,I}, s::I1, a::I2) where {A,F,I,I1<:Integer,I2<:Integer} = RLCore.estimate_reward(L.approximator, s, a)
...
function RLBase.optimise!(L::TDLearner{A,F,I}, t::Tr) where {A,F,I,Tr<:Traces}
...
function RLBase.priority(L::TDLearner{A,F,I}, transition::Tuple{T}) where {A,F,I,T}
    if L.method == :SARS
