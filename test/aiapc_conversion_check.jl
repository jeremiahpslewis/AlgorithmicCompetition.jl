using DataFrames
using AlgorithmicCompetition: AIAPCHyperParameters, AIAPCSummary, CompetitionParameters, CompetitionSolution, run_and_extract, extract_sim_results, profit_gain, AIAPCEnv
using Distributed
using ProgressMeter
using Random
using Chain
...
function test_key_AIAPC_points(; n_parameter_iterations = 1000)
    ...
    test_params = DataFrame(
        ...
        :β => [2, 0.25, 1, 0.1, 0.05, 1.75, 0.1, 1] .* 1e-5,
        :iter_min => [0, 1.5e6, 0.5e6, 0, 1.5e6, 0.2e6, 1.5e6, 0.5e6],
        :iter_max => [0.5e6, 1e7, 1.1e6, 1e7, 1e7, 0.5e6, 1e7, 1.5e6],
        :Δ_π_bar_min => [0.7, 0.75, 0.8, 0.8, 0.85, 0.7, 0.65, 0.75],
        :Δ_π_bar_max => [0.8, 0.85, 0.85, 1, 0.95, 0.8, 0.75, 0.85],
    )
    ...
    df_summary = @chain df begin
        @groupby(:α, :β)
        @combine(:Δ_π_bar = profit_gain(:π_bar, AIAPCEnv(hyperparameter_vect[1])),
